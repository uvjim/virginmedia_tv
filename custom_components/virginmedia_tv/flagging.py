...
class VirginTvFlagFile:
    ...
    def create(self, contents: Optional[str] = None) -> None:
        ...
        _LOGGER.debug(self._log_formatter.message_format("entered, include contents: %s"), contents is not None)

        if self.is_flagged():
            _LOGGER.debug(self._log_formatter.message_format("flag file already exists (%s)"), self._flag_path)
            return

        _LOGGER.debug(self._log_formatter.message_format("creating flag file: %s"), self._flag_path)
        os.makedirs(name=os.path.dirname(self._flag_path), exist_ok=True)
        with open(self._flag_path, "x") as flag_file:
            if contents is not None:
                _LOGGER.debug(self._log_formatter.message_format("writing contents to flag file"))
    ...
    def delete(self) -> None:
        ...
        _LOGGER.debug(self._log_formatter.message_format("deleting flag file: %s"), self._flag_path)
        try:
            os.remove(self._flag_path)
        except FileNotFoundError:
            _LOGGER.debug(self._log_formatter.message_format("flag file does not exist (%s)"), self._flag_path)
        else:
            _LOGGER.debug(self._log_formatter.message_format("flag file successfully removed (%s)"), self._flag_path)
